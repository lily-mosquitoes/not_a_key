...
class ReferenceDatabase(object):
    ...
    def get_abundance_matrix(self, location, sortby, sortby2):
        ...
        # total individuals for the selected time frame
        # if total is 0 no individuals of any species have been caught
        # in that time frame; if so, use 1 for calculation
        total_individuals = sum([i[1] for i in entries.items()]) or 1

        for sp, count in entries.items():

            # this is the relative abundance of each species
            # regarding the species present in that time frame;
            # species with 0 individuals in that time frame will
            # get 0 abundance; if no individuals are present in that
            # time frame, use any number (here, 1) to divide all 0 counts
            # and still show a percentage of zero
